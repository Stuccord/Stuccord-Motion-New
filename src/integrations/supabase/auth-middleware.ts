  ...
  async ({ next }) => {
    ...
    if (process.env.NODE_ENV === 'development') {
      if (!authHeader || authHeader === 'Bearer undefined' || authHeader === 'Bearer null' || authHeader === 'Bearer') {
        console.warn('[Dev Mode] Bypassing requireSupabaseAuth — using service role key to bypass RLS for mock user.');
        // Use service role key so inserts/updates are not blocked by RLS policies.
        // The mock userId is a stable dev-only UUID; RLS is skipped at the DB level.
